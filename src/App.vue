  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">关系阶段</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">情话调性</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">社交破冰与土味情话高情商指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 土味情话与社交破冰专家');
...
const metricsList = [
  {
    "key": "witClevernessRating",
    "label": "机智幽默指数"
  },
  {
    "key": "icebreakingSuccessRate",
    "label": "破冰互动成功率"
  },
  {
    "key": "sweetFlavorResonance",
    "label": "甜度撩人感染力"
  },
  {
    "key": "conversationalFlowNatural",
    "label": "聊天接话自然度"
  },
  {
    "key": "cringeFreeCheerfulness",
    "label": "油而不腻欢快分"
  }
];
const guideRules = [
  {
    "title": "配合笑眼与自嘲",
    "advice": "说土味情话时带上幽默笑脸或自我调侃，表明“我知道这很土，但我只想逗你笑”",
    "avoid": "切忌一脸严肃正经地说土味情话导致气氛僵硬"
  },
  {
    "title": "见好就收不连续轰炸",
    "advice": "土味情话是调味品而非主食，一次聊天用 1 句即可，随后顺畅切入正常话题",
    "avoid": "避免连续发几条土味情话让对方感到油腻窒息"
  },
  {
    "title": "根据对方反馈调整",
    "advice": "如果对方积极接梗可以顺势升温，若对方表现冷淡要立刻机智化解切掉话题",
    "avoid": "严禁强行说情话忽视对方的尴尬感受"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[TUWEI_SCORES\][\s\S]*?\[\/TUWEI_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[TUWEI_SCORES\](.*?)\[\/TUWEI_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('tuwei_history_records');
...
const saveHistory = () => {
  localStorage.setItem('tuwei_history_records', JSON.stringify(historyList.value));
};
